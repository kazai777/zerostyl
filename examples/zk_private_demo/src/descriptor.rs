...
fn build_inputs(w: &WitnessJson) -> CResult<ParsedInputs> {
    let collateral = parse_field(&w.collateral)
        .map_err(|e| match e {
            CircuitError::InvalidWitness(msg) => {
                CircuitError::InvalidWitness(format!("{}: {}", "collateral", msg))
            }
            other => other,
        })?;
    let collateral_nonce = parse_field(&w.collateral_nonce)
        .map_err(|e| match e {
            CircuitError::InvalidWitness(msg) => {
                CircuitError::InvalidWitness(format!("{}: {}", "collateral_nonce", msg))
            }
            other => other,
        })?;
    let threshold = parse_field(&w.threshold)
        .map_err(|e| match e {
            CircuitError::InvalidWitness(msg) => {
                CircuitError::InvalidWitness(format!("{}: {}", "threshold", msg))
            }
            other => other,
        })?;
    let collateral_commitment = PoseidonCommitmentChip::hash_outside_circuit(
        collateral,
        collateral_nonce,
    );
...
fn witness_schema_static() -> &'static WitnessSchema {
    ...
    S.get_or_init(|| WitnessSchema {
        fields: vec![
            WitnessField { name : "collateral".into(), kind : FieldType::U64, visibility
            : FieldVisibility::Private, description : None, }, WitnessField { name :
            "collateral_nonce".into(), kind : FieldType::Fp, visibility :
            FieldVisibility::Private, description : None, }, WitnessField { name :
            "threshold".into(), kind : FieldType::U64, visibility :
...
fn public_inputs_schema_static() -> &'static PublicInputsSchema {
    ...
    S.get_or_init(|| PublicInputsSchema {
        fields: vec![
            PublicInputField { name : "collateral_commitment".into(), kind :
            FieldType::Fp, description : None, }
        ],
    })
}
