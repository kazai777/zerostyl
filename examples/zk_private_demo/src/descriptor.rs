...
fn build_inputs(w: &WitnessJson) -> CResult<ParsedInputs> {
    let collateral = parse_field(&w.collateral).map_err(|e| match e {
        CircuitError::InvalidWitness(msg) => {
            CircuitError::InvalidWitness(format!("{}: {}", "collateral", msg))
        }
        other => other,
    })?;
    let collateral_nonce = parse_field(&w.collateral_nonce).map_err(|e| match e {
        CircuitError::InvalidWitness(msg) => {
            CircuitError::InvalidWitness(format!("{}: {}", "collateral_nonce", msg))
        }
        other => other,
    })?;
    let threshold = parse_field(&w.threshold).map_err(|e| match e {
        CircuitError::InvalidWitness(msg) => {
    ...
    let collateral_commitment =
        PoseidonCommitmentChip::hash_outside_circuit(collateral, collateral_nonce);
