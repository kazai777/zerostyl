...
//! `circuit.rs`):
//!
//! - `amount_commitment` = Poseidon(amount, amount_nonce) — public input 0
//! - `amount` ∈ 1000..=100000
//!
...
/// Public inputs in circuit order: `[0]` = `amount_commitment`.
/// Values are forwarded as 32-byte little-endian field representations.
pub fn public_inputs(amount_commitment: B256) -> [[u8; 32]; 1] {
    [amount_commitment.0]
}
...
pub fn deposit(host: &mut impl DepositHost, amount_commitment: B256, proof: Bytes) -> bool {
    if proof.is_empty() {
        return false;
    }
    if amount_commitment == B256::ZERO {
        return false;
    }
    if !host.verify_proof(&proof, &public_inputs(amount_commitment)) {
        return false;
    }
    let proof_hash = keccak256(&proof);
    let amount_nullifier = derive_nullifier(amount_commitment);
    if host.is_nullifier_used(amount_nullifier) {
        return false;
    }
    host.mark_nullifier_used(amount_nullifier);
    let timestamp = host.block_timestamp();
    host.emit_privacy_transaction(&PrivacyTransactionRecord {
        circuit: B256::new(CIRCUIT_ID),
        nullifier: amount_nullifier,
...
pub mod stylus_contract {
    ...
    impl DepositContract {
        pub fn deposit(&mut self, amount_commitment: B256, proof: stylus_sdk::abi::Bytes) -> bool {
            super::deposit(self, amount_commitment, proof.0.into())
        }
    }
}
