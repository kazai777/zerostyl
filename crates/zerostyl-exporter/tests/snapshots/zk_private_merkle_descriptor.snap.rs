...
fn build_inputs(w: &WitnessJson) -> CResult<ParsedInputs> {
    let leaf = parse_field(&w.leaf).map_err(|e| match e {
        CircuitError::InvalidWitness(msg) => {
            CircuitError::InvalidWitness(format!("{}: {}", "leaf", msg))
        }
        other => other,
    })?;
    let leaf_nonce = parse_field(&w.leaf_nonce).map_err(|e| match e {
        CircuitError::InvalidWitness(msg) => {
            CircuitError::InvalidWitness(format!("{}: {}", "leaf_nonce", msg))
        }
        other => other,
    })?;
    let root = parse_field(&w.root).map_err(|e| match e {
        CircuitError::InvalidWitness(msg) => {
...
fn witness_schema_static() -> &'static WitnessSchema {
    ...
    S.get_or_init(|| WitnessSchema {
        fields: vec![
            ...
            WitnessField {
                name: "siblings".into(),
                kind: FieldType::Array { kind: Box::new(FieldType::Fp), len: 32usize },
                visibility: FieldVisibility::Private,
                description: None,
            },
            WitnessField {
                name: "indices".into(),
                kind: FieldType::Array { kind: Box::new(FieldType::Bool), len: 32usize },
                visibility: FieldVisibility::Private,
                description: None,
            },
        ],
