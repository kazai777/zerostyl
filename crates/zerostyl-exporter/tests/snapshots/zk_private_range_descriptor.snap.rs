...
fn build_inputs(w: &WitnessJson) -> CResult<ParsedInputs> {
    let amount = parse_field(&w.amount)
        .map_err(|e| match e {
    ...
    let amount_nonce = parse_field(&w.amount_nonce)
        .map_err(|e| match e {
            CircuitError::InvalidWitness(msg) => {
    ...
    let amount_commitment = PoseidonCommitmentChip::hash_outside_circuit(
        amount,
        amount_nonce,
    );
    let circuit = DepositCircuit {
        amount: Value::known(amount),
        amount_nonce: Value::known(amount_nonce),
    };
...
fn witness_schema_static() -> &'static WitnessSchema {
    ...
    S.get_or_init(|| WitnessSchema {
        fields: vec![
            WitnessField { name : "amount".into(), kind : FieldType::Fp, visibility :
            FieldVisibility::Private, description : None, }, WitnessField { name :
            "amount_nonce".into(), kind : FieldType::Fp, visibility :
...
fn public_inputs_schema_static() -> &'static PublicInputsSchema {
    ...
    S.get_or_init(|| PublicInputsSchema {
        fields: vec![
            PublicInputField { name : "amount_commitment".into(), kind : FieldType::Fp,
            description : None, }
        ],
    })
}
